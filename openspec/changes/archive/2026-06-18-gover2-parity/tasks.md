...
- [x] 1.2 Reshape bridge `ExportCSV(category)` to open `runtime.SaveFileDialog(ctx, ...)`, write the CSV to the chosen path, and return nil on cancel (nil-db guard preserved)
- [x] 1.3 Add bridge test for the path-based export (write to temp file, assert contents); thin dialog wrapper left to smoke test
...
- [x] 5.3 Test: filter helper matches/clears correctly
...
- [x] 7.3 Smoke test: startup alerts, CSV export, dark-mode persistence across restart, search, New/Open DB, About/License
